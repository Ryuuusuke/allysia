import { Github, Mail, Facebook, ArrowDown, Umbrella } from "lucide-react";

export default function Home() {
	return (
		<div className="bg-black">
			<header className="bg-black text-white flex justify-center mt-32">
				<p className="text-3xl font-semibold">Welcome to Allysia!</p>
			</header>
			<div className="flex min-h-screen mt-[-9rem] bg-black flex-col items-center justify-center text-white w-screen">
				<div className="flex flex-wrap gap-4">
					<div>
						<Umbrella
							color="#007AFF"
							size={211}
							strokeWidth={0.7}
							className="absolute left-132 top-72 rotate-[-18deg] hover:animate-spin"
						/>
						<img
							src="/pandoraside.jpg"
							alt="Pandora Image"
							className="w-44 h-44 rounded-3xl"
						/>
						<div className="ml-[0.5px] mt-4 gap-4 inline-flex">
							<a href="https://github.com/Ryuuusuke">
								<Github
									size={42}
									className="border-3 p-[6px] hover:border-blue-800 rounded-full"
								/>
							</a>
							<a href="https://fedi.lecturify.net/ryuuusuke">
								<Facebook
									size={42}
									className="border-3 p-[6px] hover:border-blue-800 rounded-full"
								/>
							</a>
							<a href="mailto:alice@allysia.uno">
								<Mail
									size={42}
									className="border-3 p-[6px] hover:border-blue-800 rounded-full"
								/>
							</a>
						</div>
					</div>
					<div className="flex mt-1 flex-col gap-9">
						<div className="text-xl">
							<h2>Hi, My name is Alice!</h2>
							<h2>Leader of Allysia</h2>
						</div>
						<div className="text-[17px]">
							<p>I'm also known as Ryuuusuke</p>
							<p>I have big interests on linux</p>
							<p>and open source software.</p>
						</div>
					</div>
				</div>
				<footer className="absolute bottom-20 left-0 right-0">
					<div className="flex gap-18 items-end justify-center flex-row relative">
						<p>© 2025 allysia</p>

						<a
							className="text-white"
							href="https://send.allysia.uno"
						>
							<p className="text-white hover:underline">
								uploads
							</p>
						</a>
						<div className="relative bg-transparent flex pt-2 flex-col items-center">
							<ArrowDown
								className="absolute bg-transparent animate-bounce rotate-[25deg] bottom-6 left-[88%] -translate-x-[50%] z-50"
								size={28}
							/>
							<a href="https://keyserver.ubuntu.com/pks/lookup?search=alice@allysia.uno&op=index">
								<p className="text-white pt hover:underline cursor-pointer mt-6">
									public key
								</p>
							</a>
						</div>
					</div>
				</footer>
			</div>
		</div>
	);
}
